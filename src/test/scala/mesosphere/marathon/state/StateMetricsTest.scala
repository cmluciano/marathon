...
class StateMetricsTest extends FunSuite with Matchers with GivenWhenThen with ScalaFutures {
  implicit lazy val ec = akka.testkit.CallingThreadDispatcher
  ...
  private[this] class TestableStateMetrics(initial: Long*) extends StateMetrics {
    ...
    override def timedRead[T](f: => Future[T])(implicit ec: ExecutionContext): Future[T] = super.timedRead(f)
    override def timedWrite[T](f: => Future[T])(implicit ec: ExecutionContext): Future[T] = super.timedWrite(f)
