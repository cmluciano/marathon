...
trait StateMetrics {
  ...
  protected[this] def timedRead[T](f: => Future[T])(
    implicit ec: ExecutionContext = ExecutionContext.Implicits.global): Future[T] = readMetrics.timedFuture(f)

  protected[this] def timedWrite[T](f: => Future[T])(
    implicit ec: ExecutionContext = ExecutionContext.Implicits.global): Future[T] = writeMetrics.timedFuture(f)
