...
class AppInfoBaseData(
  ...
  lazy val tasksMap: Future[TaskTracker.AppDataMap] = {
  ...
  private[this] class AppData(app: AppDefinition) {
    ...
    lazy val tasksFuture: Future[Iterable[MarathonTask]] = tasksMap.map(_.getTasks(app.id))
    ...
    lazy val enrichedTasksFuture: Future[Seq[EnrichedTask]] = {
      ...
      val tasksByIdFuture = tasksMap.map(_.appTasks.get(app.id).map(_.taskMap).getOrElse(Map.empty))
